...
def north_korean_cipher(coded_message)
  ...
  def new_cipher(input)
    letter_decode = ""
    if (input.ord >= 101) && (input.ord <= 122)
      return letter_decode = (input.ord - 4).chr
    elsif (input.ord >= 97) && (input.ord <=100)
      return letter_decode = ((input.ord - 97) + (122 - 3)).chr
    end 
  end
  
  input.each do |x|
      if('a'..'z').to_a.include?(x)
        decoded_sentence << new_cipher(x)
      elsif (["@", "#", "$", "%", "^", "&", "*"].include?(x))
        decoded_sentence << " "
...
# This was actually pretty challenging in a couple of different ways:
# 1. it required having to think through and understand how to work with ASCII characters
# 2. it required modifications of the loops in a way that were unfamiliar with me


# The beginning first part was easier - it was the refactoring part of the challenge
# that was difficult.  I was lucky to have a good thinking partner (Sebastian) to 
# work through these issues with - I imagine it would have been pretty challenging
